...
Request.prototype.request = function(){
  console.log('INSIDE THE FRICKIN REQUEST');
...
Request.prototype._end = function() {
  ...
  req.once('response', res => {
    ...
    console.log('_END LOADING PARSER');
    if (!parser) {
      if (responseType) {
    ...
    console.log('_END BUFFER');
    let parserHandlesEnd = false;
    if (buffer) {
      // Protectiona against zip bombs and other nuisance
      let responseBytesLeft = this._maxResponseSize || 200000000;
      res.on('data', buf => {
        console.log('_END DATA')
    ...
    console.log('_END SET ERROR');
    // terminating events
    res.once('error', err => {
      console.log('_END ERROR');
      console.log(err);
      parserHandlesEnd = false;
